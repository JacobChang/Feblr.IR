...
module RobotsTxt =
    ...
    type Entry =
        ...
        static member empty =
            { userAgents = List.empty; directives = List.empty }

        static member isEmpty (entry: Entry) =
            List.isEmpty entry.userAgents && List.isEmpty entry.directives
    ...
    type RobotsTxt =
        ...
        static member empty =
            { entries = List.empty; sitemap = List.empty }
        ...
        static member isAllowed (robotsTxt: RobotsTxt) (userAgent: UserAgent) (pathname: string) : bool =
            let isMatch (basePath: string) (targetPath: string) : bool =
                 if basePath = targetPath then
                     ...
                     targetPath.StartsWith(basePath) && targetPath.[basePath.Length] = '/'
            ...
            let checkEntry allowed entry =
                ...
                | Some _ ->
                    List.fold checkDirective allowed entry.directives
                | _ ->
                    allowed
    ...
    let parseLine (line: string) : Line =
        if String.IsNullOrWhiteSpace line then
            ...
            if line.StartsWith("#") then
                ...
                let secondPart =
                    if commentStartPosition = -1 then
                        parts.[1].Trim()
                    else
                        parts.[1].Substring(0, commentStartPosition).Trim()
    ...
    let groupLine (robotsTxt: RobotsTxt) (line: Line) =
        ...
        | Directive directive ->
            ...
            { robotsTxt with entries = newEntry :: restEntries }
        | Sitemap sitemap ->
            { robotsTxt with sitemap = List.append robotsTxt.sitemap [sitemap] }

    let removeEmptyEntry (robotsTxt: RobotsTxt) : RobotsTxt =
        let newEntries =
            List.filter (not << Entry.isEmpty) robotsTxt.entries
