...
module Downloader =
    ...
    let platform =
        if RuntimeInformation.IsOSPlatform(OSPlatform.Linux) then
            ...
            if Environment.Is64BitOperatingSystem then Windows64 else Windows32
        else
            Linux
    ...
    let downloadURL (option: DownloadOption) =
        ...
        | OSX -> sprintf "%s/chromium-browser-snapshots/Mac/%d/chrome-mac.zip" option.downloadHost option.revision
        | Windows32 ->
    ...
    let getExecPath (option: DownloadOption) =
        match option.platform with
        | Linux -> sprintf "%s/chrome-linux/chrome" option.downloadFolder
        | OSX -> sprintf "%s/chrome-mac/Chromium.app/Contents/MacOS/Chromium" option.downloadFolder
        | Windows32
        | Windows64 -> sprintf "%s/chrome-win/chrome.exe" option.downloadFolder

    let download (option: DownloadOption) =
        job {
...
module DevToolsProtocol =
    ...
    type Browser =
        ...
        member this.waitForExit() = this.proc.WaitForExit()
    ...
    let attach (option: AttachOption) =
        async {
            let versionUriString = sprintf "http://127.0.0.1:%d/json/version" option.debugPort
            let request = Request.createUrl Get versionUriString
            let bodyStr =
                job {
